...
for i in range(90):
    for j in range(90):
        ...
        l += 1
        mesh[k, l] = point(a[i % 90, (j+1) % 90], i, (j, y)
        l += 1
        mesh[k, l] = point(a[i % 90, (j+1) % 90], i, j, z)

        k+=1

        l = 1

        mesh[k, l] = point(a[(i+1) % 90, (j+1) % 90], i, j, x)  # first point
        l += 1
        mesh[k, l] = point(a[(i+1) % 90, j % 90], i, j, y)
        l += 1
        mesh[k, l] = point(a[i % 90, j % 90], i, j, z)
        l += 1

        mesh[k, l] = point(a[(i+1) % 90, j % 90], i, j, x)  # second point
        l += 1
        mesh[k, l] = point(a[(i+1) % 90, j % 90], i, j, y)
        l += 1
        mesh[k, l] = point(a[(i+1) % 90, j % 90], i, j, z)
        l += 1

        mesh[k, l] = point(a[i % 90, (j+1) % 90], i, j, x)  # third point
        l += 1
        mesh[k, l] = point(a[i % 90, (j+1) % 90], i, j, y)
        l += 1
        mesh[k, l] = point(a[i % 90, (j+1) % 90], i, j, z)
